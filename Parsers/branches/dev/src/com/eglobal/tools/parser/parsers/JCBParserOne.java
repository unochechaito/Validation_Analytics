...
public class JCBParserOne extends FieldLoader implements IParser {
    @java.lang.SuppressWarnings("all")
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(JCBParserOne.class);
    private static JCBParserOne __INSTANCE;
    private boolean isLastField = false;

    private JCBParserOne() {
        init("resources/parsers/fieldJCB.json");
    }

    public static JCBParserOne getInstance() {
        if (__INSTANCE == null) {
            __INSTANCE = new JCBParserOne();
        }
        return __INSTANCE;
    }
        ...
        if ("fixed".equalsIgnoreCase(fieldFormat.getLengthIndicator())) {
            ...
            if (matcher.find()) {
                ...
                if (buffer.remaining() >= lengthBytes.length && !isLastField) {
                    ...
                    if ("BCD".equalsIgnoreCase(fieldFormat.getFormat()) && "Receiving institution identification code".equalsIgnoreCase(fieldFormat.getName())) {
                        numBytes = Integer.parseInt(Converter.bcdValue(lengthBytes));
                    } else if ("BCD".equalsIgnoreCase(fieldFormat.getFormat()) && !"IAD".equalsIgnoreCase(fieldFormat.getName())) {
                        numBytes = Integer.parseInt(Converter.bcdValue(lengthBytes)) / 2;
                    } else if ("OCT".equalsIgnoreCase(fieldFormat.getFormat())) {
                        numBytes = Integer.parseInt(Converter.bcdValue(lengthBytes), 8);
                    } else if ("HEX".equalsIgnoreCase(fieldFormat.getFormat())) {
                        numBytes = Integer.parseInt(Converter.byteArrayAsHexString(lengthBytes), 16);
                    } else 
//                    } else if ("DEC".equalsIgnoreCase(fieldFormat.getFormat())) {
//                        numBytes = Integer.parseInt(Converter.byteArrayAsHexString(lengthBytes));
                ...
                } else if (isLastField && "Receiving institution identification code".equalsIgnoreCase(fieldFormat.getName()) && buffer.remaining() != lengthBytes.length) {
                    buffer.get(lengthBytes);
                    log.debug("length bytes " + Converter.byteArrayAsHexString(lengthBytes));
...
//            case "DEC":
//                try {
//                    String hexString = Converter.byteArrayAsHexString(rawData);
//                    int midLength = hexString.length() / 2;
//                    value = String.valueOf(Integer.parseInt(hexString.substring(0, midLength), 10));
////                    value = String.valueOf(Integer.parseInt(Converter.byteArrayAsHexString(rawData), 10));
//                } catch (NumberFormatException e) {
//                    log.error("Number format exception while parsing DEC field: {}", e.getMessage(), e);
//                }
//                break;
        case "Cp1047": 
            try {
...
//    }
    public static void main(String[] args) {
        byte[] ebcdicData = new byte[] {(byte) 193, (byte) 199, (byte) 213, (byte) 226};
