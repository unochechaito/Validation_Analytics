...
//    final int POSITION_DIFFERENCES = 4;
    ...
    private static final List<String> FIRST_COLUMN_ALIGN_OPTIONS = Arrays.asList("Campos presentes por tipo de transaccion recibidos de adquirente MasterCard", "Campos presentes por tipo de transaccion recibidos de adquirente Visa", "Campos presentes por tipo de transacción enviados al emisor Visa", "Campos presentes por tipo de transaccion enviados al emisor MasterCard", "Campos presentes respondidos por tipo de transaccion al adquirente Visa", "Campos presentes respondidos por tipo de transaccion al adquirente Mastercard", "Campos presentes por tipo de transaccion respondidos por el emisor Visa", "Campos presentes por tipo de transaccion respondidos por el emisor MasterCard", "Tags emv por tipo de transaccon recibidos de adquirente Visa", "Tags emv por tipo de transaccon recibidos de adquirente MasterCard", "Tags emv por tipo de transaccon enviados al emisor Visa", "Tags emv por tipo de transaccon enviados al emisor MasterCard");
    private static final List<String> SECOND_COLUMN_ALIGN_OPTIONS = Arrays.asList("Campos presentes por tipo de transaccion recibidos de adquirentes Nacional", "Campos presentes por tipo de transaccion enviados a emisores Nacional", "Campos presentes por tipo de transacción respondidos por emisores Nacional", "Campos presentes por tipo de transaccion respondidos a adquirentes Nacional", "Tokens presentes por tipo de transaccion recibidos de adquirentes Nacional", "Tokens presentes por tipo de transaccion enviados a emisores Nacional", "Tokens presentes por tipo de transaccion respondidos a adquirentes Nacional", "Tokens presentes por tipo de transaccion respondiso de emisores Nacional", "Mapeo MTI-ProcCode a TranCode");
...
//                    data[POSITION_DIFFERENCES] = "";
    ...
    private Alert getAlertForC39(File file) throws IOException {
        ...
        if (errorEmiIndex == -1 || errorAdqIndex == -1) {
            throw new IllegalArgumentException("Las columnas \'error_emi\' o \'error_adq\' no estan presentes en el archivo.");
        }
    ...
    private Alert getAlertForOn2Desc(File file) throws IOException {
        ...
        if (on2Index == -1 || percentageIndex == -1) {
            throw new IllegalArgumentException("Las columnas \'on2\' o \'porcentaje\' no estan presentes en el archivo.");
        }
        for (int i = 1; i < lines.size(); i++) {
            String[] fields = lines.get(i).split(",");
    ...
    private static double getPorcentajeConError(int on2Index, int porcentajeOn2BinIndex, List<String> lines) {
        if (on2Index == -1 || porcentajeOn2BinIndex == -1) {
            throw new IllegalArgumentException("Las columnas \'on2\' o \'porcentaje_on2_bin\' no estan presentes en el archivo.");
        }
        int acumulado = 0;
    ...
    private Alert getAlertForPorcentajeC39PorDispatcher(File file) throws IOException {
        ...
        if (dispatcherEmiIndex == -1 || c39EmiIndex == -1 || porcentajeC39DispatcherIndex == -1) {
            throw new IllegalArgumentException("Las columnas \'dispatcher_emi\', \'C39_emi\', o \'porcentaje_C39_dispatcher\' no estan presentes en el archivo.");
        }
        int acumulado = 0;
        int totalRegistros = 0;
