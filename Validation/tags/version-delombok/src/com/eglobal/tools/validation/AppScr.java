...
//             DBManager.getInstance().deleteDB();
        ...
        env = options[opt];
        frame.setTitle("Herramienta de validación (" + options[opt] + ")");
        ...
        cerrarMenuItem.addActionListener(e -> {
            log.info("Cerrando la aplicación desde el menú...");
            WindowEvent closingEvent = new WindowEvent(frame, WindowEvent.WINDOW_CLOSING);
            Toolkit.getDefaultToolkit().getSystemEventQueue().postEvent(closingEvent);
        });
        JMenu reportesMenu = new JMenu("Fotos");
        menuBar.add(reportesMenu);
        JMenuItem generacionFotosMenuItem = new JMenuItem("Generación de Fotos");
...
//    }
    private void openPhotosWindow() {
        JFrame photosFrame = new JFrame("Generación de Fotos");
        ...
        for (File file : files) {
            ...
            if (file.isFile() && file.getName().endsWith(".db") && !file.getName().equals("validation.db")) {
                if (file.delete()) {
                    ...
                    log.error("No se pudo eliminar el archivo (¿está en uso?): " + file.getName());
                }
            }
        }
    }
